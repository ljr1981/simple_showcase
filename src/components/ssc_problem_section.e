...
feature {NONE} -- Content
	...
	section_content: STRING
		...
		do
			...
			l_content.class_ ("mb-16 opacity-0")
				.x_show ("visible")
			...
			l_stats_grid.class_ ("grid md:grid-cols-2 lg:grid-cols-4 gap-6")

			l_stats_grid.raw_html (stat_card ("41%%", "more bugs with Copilot", "Uplevel 2024", 1))
			l_stats_grid.raw_html (stat_card ("45%%", "AI code has security flaws", "Veracode 2024", 2))
			l_stats_grid.raw_html (stat_card ("33%%", "developers trust AI code", "Stack Overflow 2025", 3))
			l_stats_grid.raw_html (stat_card ("85%%", "Devin AI tasks failed", "Independent audit", 4))

			l_container.raw_html (l_stats_grid.to_html)
	...
	stat_card (a_number, a_label, a_source: STRING; a_index: INTEGER): STRING
		...
		do
			l_card := alpine.div
			l_card.class_ ("p-6 rounded-lg bg-white/10 text-center opacity-0")
				.x_show ("visible")
